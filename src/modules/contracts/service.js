...
export const rejectContract = async (contractId, employeeId, contractReasonRejectDto) => {
    try{
        const response = await api.put(`/contracts/${contractId}/reject/${employeeId}`, contractReasonRejectDto);
        return response.data;
    }
    catch(error){
        console.error('Call API Error:', error);
